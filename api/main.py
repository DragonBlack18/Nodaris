...
from api.availability_service import (
    AvailabilityService,
)
from api.device_state_repository import (
    DeviceStateRepository,
)
from api.diagnostic_service import (
    DiagnosticService,
)
from api.probe_history_repository import (
    ProbeHistoryRepository,
)
from api.probe_availability_service import (
    ProbeAvailabilityService,
)
...
probe_history_repository = (
    ProbeHistoryRepository()
)

probe_availability_service = (
    ProbeAvailabilityService()
)

incident_observation_service = (
    IncidentObservationService()
)
...
app = FastAPI(
    title="NODARIS API",
    description=(
        "API central de monitoramento "
        "de infraestrutura do NODARIS."
    ),
    version="0.3.0",
    lifespan=lifespan,
)
...
def _calculate_scan_freshness(
    ...
    # 3 ciclos configurados + 15 segundos
    ...
    if last_scan_at:
        ...
        try:
            ...
            # Mudança de relógio do Windows não deve
            # produzir idade negativa.
            ...
            # Timestamp inválido significa que não
            # conseguimos avaliar freshness.
            last_scan_age_seconds = None
            stale = None

    return {
        "scan_interval_seconds": round(
            interval,
            2,
        ),
        "last_scan_age_seconds": (
            ...
            if last_scan_age_seconds
            is not None
            else None
        ),
        "stale_after_seconds": round(
            stale_after_seconds,
            2,
        ),
        "stale": stale,
...
async def health():
    ...
    try:
        ...
        # O health precisa continuar disponível
        # mesmo se a configuração não puder
        # ser lida temporariamente.
...
@app.get(
    "/api/v1/integrations/blackbox/health"
)
...
async def get_devices():
    ...
    try:
        ...
        devices = (
            devices_repository.get_devices()
        )
...
async def ping(ip: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail="Endereço IPv4 inválido.",
        )

    return await monitor_service._probe(
        ip
    )
...
async def incidents(
    ...
    try:
        ...
        items = (
            incident_repository.get_incidents(
                limit=limit,
                status=status,
            )
        )
    ...
    return (
        ...
        .enrich_collection(
            {
                "summary": (
                    incident_repository.get_summary()
                ),
                "returned": len(items),
                "incidents": items,
            }
...
@app.get(
    "/api/v1/diagnostics/{ip}"
)
async def diagnostic_by_ip(
    ip: str,
):
